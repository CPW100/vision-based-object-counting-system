...
r"""
input_type -> default_value = 'image_tensor', 
...
side_input_types -> value='', If use_side_inputs is True, this explicitly sets 
                              the type of the side input tensors. The 
                              dimensions are to be provided as a comma-separated list 
                              of types, each of `string`, `integer`, or `float`. 
                              This flag is required if using side inputs.

side_input_names -> value='', If use_side_inputs is True, this explicitly sets 
...
class export_fine_tuned_model():
    def __init__(self):
        
        self.pipeline_config_path = None
        self.trained_checkpoint_dir = None
        self.output_directory = None
        self.input_type = 'image_tensor'
        self.config_override = ''
        self.use_side_inputs = False
        self.side_input_shapes = ''
        self.side_input_types = ''
    ...
    def run(self):
      ...
      text_format.Merge(self.config_override, pipeline_config)
      exporter_lib_v2.export_inference_graph(
          self.input_type, pipeline_config, self.trained_checkpoint_dir,
          self.output_directory, self.use_side_inputs, self.side_input_shapes,
          self.side_input_types, self.side_input_names)
