...
def draw_bounding_box_on_image_array(image,
  ...
  Args:
    ...
    xmax: xmax of bounding box.font
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  centre, box_area, bbox_coord = draw_bounding_box_on_image(image_pil, ymin, xmin, ymax, xmax, fontsize,
                             color,thickness, display_str_list,
                             use_normalized_coordinates)
  np.copyto(image, np.array(image_pil))
  return centre, box_area, bbox_coord
...
def draw_bounding_box_on_image(image,
  ...
  if use_normalized_coordinates:
    ...
    box_area = (abs(left-right)*abs(top-bottom))
    bbox_coord = (left, right, top, bottom)
  else:
  ...
  if thickness > 0:
    draw.line([(left, top), (left, bottom), (right, bottom), (right, top),
              ...
              fill=color)
  try:
    font =  ImageFont.truetype(fm.findfont(fm.FontProperties(family='DejaVu Sans')),fontsize)    # ImageFont.truetype('arial.ttf', 100)
  except IOError:
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for box, color in box_to_color_map.items():
    ...
    centre, box_area, bbox_coord = draw_bounding_box_on_image_array(
        image,
        ymin,
        xmin,
        ymax,
    ...
    temp = [box_to_display_str_map[box], centre, box_area, bbox_coord]
    my_label_list.append(temp)
  return image, my_label_list
